"""Pattern detection screen: bridges limit_up pattern system with sz project.

Loads the PatternLibrary, runs PatternDetector on tushare-format data,
and provides a screener-compatible interface.
"""
...
def _ensure_detector(stock_data_sz_format):
    ...
    lib_path = config.PATTERN_LIBRARY_PATH
...
def run_pattern_scan(stock_data, date):
    ...
    if _detector_date is None:
        ...
        if target_idx is not None and target_idx > config.PATTERN_WARMUP_DAYS:
            warmup_dates = all_dates[target_idx - config.PATTERN_WARMUP_DAYS:target_idx]
